    ...
    public class ChannelsController : ControllerBase
    {
        ...
        public async Task<IActionResult> EletronicChannels([FromBody] ApiRequest request)
        {
            try
            {
                ...
                var response = await _channelsService.GetEletronicChannels(request);
                ...
                if (response.ResponseCode == 200)
                {
                    var channelsDTO = response as ElectronicChannelsDTO;

                    return this.Ok(new ElectronicChannelResponse(channelsDTO.ElectronicChannelResponse));
                }
                else
                {
                    return StatusCode(response.ResponseCode, new ResponseError()
